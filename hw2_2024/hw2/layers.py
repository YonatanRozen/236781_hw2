...
class TanH(Layer):
    ...
    def backward(self, dout):
        ...
        x = self.grad_cache["x"]
        y = torch.ones_like(x)
        y = 1 - ((torch.exp(x) - torch.exp(-x)) / (torch.exp(x) + torch.exp(-x))) ** 2
        dx = y * dout
        # ========================
...
class Linear(Layer):
    ...
    def __init__(self, in_features, out_features, wstd=0.1):
        ...
        self.b = torch.zeros(self.w.shape[0])
        # ========================
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        # ====== YOUR CODE: ======
        nrows = x.shape[0]
        true_scores = -x[torch.arange(nrows), y]
        exp_x = torch.exp(x)
        log_exp_x = torch.log(torch.sum(exp_x, dim=1))
        loss = (true_scores + log_exp_x).mean()
        # ========================
    ...
    def backward(self, dout=1.0):
        ...
        C = x.shape[1]
        res = torch.exp(x)
        temp = torch.sum(res, dim=1, keepdim=True)
        res = res / temp
        res[torch.arange(N), y] -= 1
        dx = dout * res / N
        # ========================
...
class Sequential(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        # do backward pass until the input layer
        din = dout
